...
#dbname='awsgypsy_s3_dups_tmp_db' user='awsgypsy' host='aa1bbuxq2v7x37y.cndqisrlueee.us-west-2.rds.amazonaws.com'  password='kfKPwmAtvL'

def main():
	dbuser = 'awsgypsy'
	dbpass = 'kfKPwmAtvL'
	#stackinfo = createcloudformation(dbuser,dbpass)
	walk_buckets('aa1bbuxq2v7x37y.cndqisrlueee.us-west-2.rds.amazonaws.com',dbuser,dbpass)
...
def walk_buckets(dbhost,dbuser,dbpass):
	...
	for bucket in s3_conn.list_buckets()['Buckets']:
		...
		cursor.execute("INSERT INTO s3objects (account, bucket, path , filename , isdir , md5sum , size ) VALUES ('default', %s, %s, %s, 'FALSE', %s )", ( [ bucketname, path.encode('ascii', 'ignore'), filename.encode('ascii', 'ignore'),  md5sum, str(obj['Size']) ] ) )
